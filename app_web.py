...
# --- TAB: CLASIFICACIÓN (Versión con Escudos y nombres limpios) ---
with tabs[0]:
    with get_db_connection() as conn:
        # Paso 1: Traemos nombre y escudo de la tabla
        df_eq = pd.read_sql_query("SELECT nombre, escudo FROM equipos WHERE estado = 'aprobado'", conn)
        
        if df_eq.empty: 
            st.info("No hay equipos todavía.")
        else:
            # LIMPIEZA: Forzamos a que el nombre sea texto y no una tupla
            ...
            escudos_map = dict(zip(df_eq['nombre'], df_eq['escudo']))

            # Paso 2: Cálculo de Estadísticas (Tu lógica original)
            df_p = pd.read_sql_query("SELECT * FROM partidos WHERE goles_l IS NOT NULL", conn)
            for _, f in df_p.iterrows():
                # Limpiamos nombres de locales y visitantes también
            ...
            # Paso 3: Construcción del DataFrame Final
            df_f = pd.DataFrame.from_dict(stats, orient='index').reset_index()
            df_f.columns = ['Equipo', 'PJ', 'PTS', 'GF', 'GC'] # "EQ" cambiado a "Equipo"
            df_f['DG'] = df_f['GF'] - df_f['GC']
            df_f = df_f.sort_values(by=['PTS', 'DG', 'GF'], ascending=False).reset_index(drop=True)
            df_f.insert(0, 'POS', range(1, len(df_f) + 1))

            # Paso 4: Renderizado HTML Épico
            # Inyectamos el estilo para que el escudo se vea pequeño pero nítido
            st.markdown("""
                <style>
                    .escudo-tabla { width: 30px; height: 30px; object-fit: contain; vertical-align: middle; margin-right: 12px; }
                    ...
                    .mobile-table th { border-bottom: 2px solid #ddd; padding: 8px; }
                    .mobile-table td { border-bottom: 1px solid #eee; padding: 10px; text-align: center; }
                </style>
            """, unsafe_allow_html=True)
            ...
            for _, r in df_f.iterrows():
                url = escudos_map.get(r['Equipo'])
                
                # Si Fifinhos (o cualquier otro) tiene URL, la mostramos. Si no, espacio de 30px.
                if url and str(url).strip() != "" and str(url) != "None":
                    img_tag = f'<img src="{url}" class="escudo-tabla">'
                else:
                    img_tag = '<div style="width: 30px; display: inline-block; margin-right: 12px;"></div>'

                html += f"""
                <tr>
                    <td>{r['POS']}</td>
                    <td style='text-align:left'><div class='celda-equipo'>{img_tag} {r['Equipo']}</div></td>
                    <td><b>{r['PTS']}</b></td>
                    <td>{r['PJ']}</td>
                    <td>{r['GF']}</td>
                    <td>{r['GC']}</td>
            ...
            # EL CIERRE: st.markdown con unsafe_allow_html=True para dibujar la tabla
